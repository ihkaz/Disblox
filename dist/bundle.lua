...
function Gateway:connect()
    ...
    self.ws.OnMessage:Connect(function(msg)
        ...
        elseif data.op == OP_INVALID_SESSION then
            warn("[GATEWAY]", { status = "invalid_session" })
            wait(5)
            self:identify()
        end
    end)
    ...
    self.ws.OnClose:Connect(function()
        warn("[GATEWAY]", { status = "disconnected" })
    ...
    print("[GATEWAY]", { status = "connected" })
end
...
function CommandHandler:registerCommand(options)
    ...
    print("[COMMAND]", { status = "registered", name = command.name })
end
...
function CommandHandler:registerAll()
    ...
    print("[COMMANDS]", { status = "registered", count = #commandsData })
end
...
function CommandHandler:registerButton(customId, handler)
    ...
    print("[BUTTON]", { status = "registered", customId = customId })
end
...
function CommandHandler:handleSlashCommand(interaction)
    ...
    if command then
        ...
        print("[COMMAND]", {
            name = commandName,
            userId = user and user.id or nil,
            username = user and user.username or nil
        })
        ...
        warn("[COMMAND]", { status = "missing_handler", name = commandName })
    end
end
...
function CommandHandler:handleButton(interaction)
    ...
    if handler then
        ...
        warn("[BUTTON]", { status = "missing_handler", customId = customId })
    end
end
...
function Client.new(options)
    ...
    self.gateway:on("READY", function(data)
        self.user = data.user
        print("[READY]", {
            username = data.user and data.user.username or nil,
            userId = data.user and data.user.id or nil
        })
        self:emit("ready")
